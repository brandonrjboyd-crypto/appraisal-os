export default async function handler(req, res) {
  ...
  const disputeLines = (disputes || []).map(d => 
    `- ${d.category || d.description}: Carrier $${d.cTotal || 0}, PH $${d.phTotal || 0}, Award $${awardValues?.[d.id] || d.cTotal || 0}`
  ).join('\n');
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `You are a neutral insurance appraisal umpire drafting a binding appraisal award.
